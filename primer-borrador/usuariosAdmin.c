...
///El email debe contener un @ y un .com. Adem�s se debe v�lidar que el email no este registrado previamente por otro usuario.
///La contrase�a debe contener por lo menos 1 may�scula y 1 min�scula.
///Al momento en que finaliza el proceso de registro, se lo debe redirigir a la pantalla de login.

///Un usuario debe poder ingresar al sistema con sus credenciales: email y contrase�a. En caso de que el email no exista
///se le debe indicar que no existe usuario con esos datos. En caso de que la contrase�a no sea correcta, se le debe indicar
/// que algunos de los campos son incorrectos.

///Un usuario una vez logueado en el sistema podr� modificar su informaci�n personal.

///Una opci�n de deslogueo para salir del sistema y que pueda ingresar con otra cuenta.

///Asimismo, adem�s de las funcionalidades anteriores, un usuario administrador podr�:
...
void registrarUsuario(nodoUsuarios * listaUsuarios)
{
    ...
    do {
        ...
        fflush(stdin);
        gets(username);
        rta = buscarUsuarioPorUsername(listaUsuarios, username);  // Implementar esta funci�n
        ...
        if (rta == 1) {
            ...
            if (opcion == 2) {
                ...
                return;  // Salimos de la funci�n si elige salir
            }
        }
    ...
    do {
        ...
        fflush(stdin);
        gets(email);
        rta = buscarUsuarioPorEmail(listaUsuarios, email);  // Implementar esta funci�n
...
stUsuario cargarContraseniaRegistro(stUsuario usuario)
{
    ...
    while(!validarContrasenia(contrasenia, confirmacion) || || !validarMayusculaMinuscula(contrasenia))
    {
         if (!validarContrasenia(contrasenia, confirmacion)) {
            ...
            printf("La contrasenia debe tener al menos una may�scula y una min�scula\n");
        }
    ...
    // Si todo est� bien, se copia la contrase�a al usuario
...
stUsuario cargarUsuarioRegistro(int tipoUsuario)
{
    ...
    {
        printf("Email inv�lido. Debe contener '@' y terminar en '.com'.\n");
    }
